  ...
  return (
    <nav style={{ background: '#fff', borderBottom: '1px solid #EDE6DA', padding: '0 2rem', height: 64, display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100, fontFamily: "'Montserrat', sans-serif" }}>
      ...
      <div onClick={() => navigate('/')} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
        <div style={{ width: 38, height: 38, borderRadius: '50%', border: '1.5px solid #A06535', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'Playfair Display, serif', fontSize: 13, fontWeight: 700, color: '#A06535' }}>GCW</div>
        <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
        {links.map(link => (
          <button key={link.path} onClick={() => navigate(link.path)} style={{ padding: '6px 14px', border: 'none', borderRadius: 6, background: isActive(link.path) ? 'rgba(184,115,74,0.1)' : 'transparent', color: isActive(link.path) ? '#A06535' : 'rgba(43,43,43,0.6)', fontSize: 13, fontWeight: isActive(link.path) ? 600 : 500, cursor: 'pointer', fontFamily: "'Montserrat', sans-serif", transition: 'all 0.15s' }}
